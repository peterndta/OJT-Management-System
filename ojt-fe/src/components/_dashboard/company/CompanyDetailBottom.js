...
const CompanyDetailBottom = (props) => {
  const { id } = useParams();
  const { user } = props;
    ...
    const getCompanies = async () => {
      try {
        ...
        const response = await axiosPrivate.get(
          `/admin/get-jobs-by-company-id/${id}`,
          {
      ...
      <CardContent sx={{ mb: 10 }}>
        <Grid container spacing={3}>
          ...
          <Grid item md={12} xs={12}>
            <Typography variant="body2" style={{display: 'inline'}}>{user.description}</Typography>
          </Grid>
        </Grid>
      </CardContent>
    </Card>
