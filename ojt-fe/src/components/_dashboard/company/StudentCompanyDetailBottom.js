...
const StudentCompanyDetailBottom = (props) => {
    const { id } = useParams();
    const { user } = props;
        ...
        const getCompanies = async () => {
            try {
                ...
                const response = await axiosPrivate.get(
                    `/student/get-jobs-by-account-id-and-company-id/${id}`,
                    {
                ...
                <Grid container spacing={3}>
                    ...
                    {jobs?.length ? (
                        ...
                        >
                            {jobs.map((job, i) => (
                                <Link
                                    to={`/student/dashboard/jobs/${job.id}`}
                                    color="inherit"
                                    underline="none"
...
StudentCompanyDetailBottom.propTypes = {
    user: PropTypes.object,
};
export default StudentCompanyDetailBottom;
