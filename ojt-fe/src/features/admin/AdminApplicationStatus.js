// material
import {
  Box, Card, CircularProgress, Container, FormControl,
  ...
  if (query) {
    ...
    if (filterBy === 6) {
      return filter(
        array,
        (_user) =>
          _user.gpa.toString().indexOf(query.toString()) !== -1
      );
    }
  }
...
export default function AdminApplicationStatus() {
  ...
  useEffect(() => {
    ...
    const getStudents = async () => {
      try {
        setLoading(true);
        const response = await axiosPrivate.get("/admin/get-all-application", {
