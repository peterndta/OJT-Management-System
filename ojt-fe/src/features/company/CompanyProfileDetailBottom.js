...
const CompanyProfileDetailBottom = (props) => {
    const { id } = useParams();
    const { user } = props;
    ...
    useEffect(() => {
        ...
        const getCompanies = async () => {
            try {
                setLoading(true);
                const response = await axiosPrivate.get('companies/get-list-jobs-by-company-id', {
                    ...
                    </Grid>
                    {
                        jobs?.length ? (
                            ...
                            >
                                {jobs.map((job, i) => (
                                    <Link
                                        to={`/company/dashboard/jobs/${job.id}`}
                                        color="inherit"
                                        underline="none"
...
CompanyProfileDetailBottom.propTypes = {
    user: PropTypes.object,
};
export default CompanyProfileDetailBottom;
